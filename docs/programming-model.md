...
The v0.11 implementation has a `LogicalJobSpec` one level above `CooperativeJobSpec`. A logical role may offer multiple named Task implementations. Numeric `shards: N` preserves fixed independent contributions. An auto policy instead declares one explicit input as partitionable, an upper bound, per-child overhead, and a minimum modeled gain. `byte_range` uses uniform byte slices. `records` accepts an explicit record-offset index tied to a published immutable Object, partitions by record count, and maps each child back to an exact byte range. `job plan` evaluates candidate counts and per-shard implementation/resource choices using predicted Resource availability and range-sized network transfers.

`job auto-submit` does not execute the preview as a frozen compilation. When dependencies complete, fixed sharding materializes the declared contributions, while auto sharding re-evaluates its count against current membership/topology and the real partition input size/location. Each auto child carries `TaskShard { index, count, partition }`; a record partition includes record start/count as well as the concrete byte range used by the data plane. Selected implementations/resources remain advisory and every child still goes through normal lease-time scheduling. A sharded role completes only after all children complete, with deterministic downstream Object order.

When `reduction` is present, “all children complete” transitions the role into `Reducing` rather than directly to `Completed`. Each reduction level is built from the real current Object locations. Contiguous pairwise-local groups reduce first; if none are local enough, ordered groups up to `max_fan_in` cross the wider topology. The reducer must be Pure, have no static inputs, and obey ordered associativity. Reducer lease expiry requeues only that internal Task.
...
Role decomposition and partition semantics remain explicit. v0.11 can choose a shard count for raw byte ranges or an application-supplied record boundary index. It does not infer record boundaries from payload bytes, tensor axes, training-example semantics, or arbitrary program decomposition. A sharded role may declare a Pure ordered-associative reducer; Plurifold may regroup adjacent inputs into a topology-aware hierarchy but never reorder them. Commutative/quorum/streaming reductions and arbitrary fan-out/fan-in rewrites remain future work.
