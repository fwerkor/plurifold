...
Plurifold v0.11 uses a deliberately small **HTTP/JSON control plane** (protocol constant `API_VERSION = 7`). The Rust request/response types in `plurifold-protocol` are authoritative for the current prototype. `proto/plurifold.proto` remains a transport-neutral design sketch for a possible future binary RPC encoding; it is not generated into the build.
...
`TaskSpec` now has an optional runtime-generated `pipeline`. A `TaskPipeline` is an ordered list of stages. Stage inputs explicitly bind either to an external Task input index or to the immediately previous stage output. Ordinary submitted Tasks leave this field absent. The v0.11 agent uses it for fused builtin-family work: all external Objects are materialized once, stage intermediates remain in memory, and only the final bytes are committed as the Task output.

`TaskSpec` also has optional shard context. Fixed numeric `LogicalRoleSpec.shards` remains wire-compatible with earlier APIs and produces `TaskShard { index, count, partition: null }`. API 7 supports auto policies with `mode: "auto"`, `max_shards`, `partition`, optional `per_shard_overhead_ms`, and optional `min_gain_ratio`. `partition.kind` may be `byte_range` or `records`. A record policy carries `input` plus `offsets[]`; the referenced Object must already be published, offsets start at 0, are strictly increasing, and end at Object size. Generated record children carry `record_start`, `record_count`, `total_records`, and the concrete `offset/length/total_bytes` used for transport. A valid shard still requires `count > 0`, `index < count`, a valid input index, and an in-bounds range.
...
`GET /v1/jobs/{id}` returns either the fixed `CooperativeJobSpec` or live `LogicalJobSpec`, plus each role's state. For an ordinary dynamically selected role it reports the chosen implementation, advisory resource, ready-time estimated cost, and optional fusion metadata. A sharded role reports `Sharded { tasks, completed }` while child work is active. If it declares a reducer, completion then transitions through `Reducing { tasks, completed, level }`; those Task IDs are the current internal reduction level. The role becomes `Completed([...])` only when one final reduction Object remains. The `shards[]` view continues to expose child partitions/status. In API 7, fusion metadata still contains `chain_roles`, `stage_index`, `estimated_avoided_transfer_ms`, and `estimated_vs_separate_ms`.

## Logical planning

`POST /v1/jobs/plan` accepts a `LogicalJobSpec` whose roles contain alternative Task implementations, fixed/automatic shard policies, and optional reducers. It returns a `CooperativePlan` with `roles[].shards[]` plus optional `roles[].reductions[]`. Every predicted reduction level contains its level number and reducer placements/input counts. The response estimated makespan includes the reduction hierarchy. Live execution remains a separate ready-time decision.
...
Bulk object bytes do **not** pass through the coordinator. In v0.11 each agent exposes:
...
The v0.11 service is **unauthenticated** and defaults to loopback. Cross-trust-domain authentication, authorization, TLS, signed artifacts, and secret locality are design requirements in `security.md`, not claims of the current implementation.
